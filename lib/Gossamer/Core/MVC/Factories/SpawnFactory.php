...
class SpawnFactory
{
     ...
     * @param  string  $name
     *
     * @return \Gossamer\Essentials\Configuration\SiteParams|mixed|void
        ...
        if ($constructor != null) {
            foreach ($constructor->getParameters() as $parameter) {
                if (!is_null($parameter->getType())
                    && !is_null($parameter->getType()->getName())
                ) {
     ...
     * @param  string  $name
     *
     * @return \Gossamer\Essentials\Configuration\SiteParams|mixed|null
     * @throws \Gossamer\Set\Exceptions\ObjectNotFoundException
     */
     ...
     * @throws \ReflectionException
     */
    public function getArguments($object, string $method) : array
    {
        ...
        foreach ($parameters as $parameter) {
            ...
            if (!is_null($parameter->getType()->getName())) {
                $arguments[$parameter->getType()->getName()]
                    = $this->spawn($parameter->getType()->getName());
            }
        }
    ...
    public function createArgumentMetadata($controller): array
    {
        ...
        if (is_array($controller)
            && method_exists($controller[0], $controller[1])
        ) {
            $reflection = new ReflectionMethod($controller[0], $controller[1]);
        } elseif (is_object($controller) && !$controller instanceof Closure) {
            $reflection
                = (new ReflectionObject($controller))->getMethod('__invoke');
        ...
        foreach ($reflection->getParameters() as $param) {
            ...
            $arguments[] = new ArgumentMetadata($param->getName(),
                self::getType($param, $reflection), $param->isVariadic(),
                $param->isDefaultValueAvailable(),
                $param->isDefaultValueAvailable() ? $param->getDefaultValue()
                    : null, $param->allowsNull(), $attributes);
        }
    ...
    private function getType(
        ...
        if ($function instanceof ReflectionMethod) {
            ...
            switch ($lcName) {
                ...
                case 'parent':
                    return ($parent = $function->getDeclaringClass()
     ...
     * @throws \Gossamer\Set\Exceptions\ObjectNotFoundException
     */
    private function injectTraits($object) : void
    {
        ...
        if (in_array('Gossamer\Core\System\Traits\EntityManagerTrait',
            $traits)
        ) {
